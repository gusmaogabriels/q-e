...
!civn 
  CALL reshape_psi(psi, kdimx, nact, nbnd, act_idx, buffer, 1) 
  CALL reshape_psi(w, kdimx, nact, nbnd, act_idx, buffer, 1) 
  CALL reshape_psi(hpsi, kdimx, nact, nbnd, act_idx, buffer, 1) 
  CALL reshape_psi(spsi, kdimx, nact, nbnd, act_idx, buffer, 1) 
!
...
!civn 
        ...
        if (overlap) then
           ...
           CALL ZGEMM('C','N', l, l, kdim, C_ONE, w(1,(j-1)*sbsize + 1), kdimx, sw(1,(j-1)*sbsize + 1), kdimx, C_ZERO, M(l+1, l+1 ), sbsize3)
        else
           !call threaded_assign( buffer1, buffer, kdimx, l )
           CALL ZGEMM('C','N', l, l, kdim, C_ONE, w(1,(j-1)*sbsize + 1), kdimx, w(1,(j-1)*sbsize + 1), kdimx, C_ZERO, M(l+1, l+1 ), sbsize3)
        end if
!civn (moved up)
...
!civn 
        ...
        CALL ZGEMM('C','N', l, l, kdim, C_ONE, buffer, kdimx, hw(1,(j-1)*sbsize + 1), kdimx, C_ZERO, K(1, l+1), sbsize3)
        !
        if (overlap) then
           !call threaded_assign( buffer1, sw, kdimx, l, col_idx )
           CALL ZGEMM('C','N', l, l, kdim, C_ONE, buffer, kdimx, sw(1,(j-1)*sbsize + 1), kdimx, C_ZERO, M(1, l+1), sbsize3)
        else
           !call threaded_assign( buffer1,  w, kdimx, l, col_idx )
           CALL ZGEMM('C','N', l, l, kdim, C_ONE, buffer, kdimx, w(1,(j-1)*sbsize + 1), kdimx, C_ZERO, M(1, l+1), sbsize3)
        end if
!civn 
...
!civn 
          ...
          CALL ZGEMM('C','N', l, l, kdim, C_ONE, buffer, kdimx, hp(1,(j-1)*sbsize + 1), kdimx, C_ZERO, &
                                                                                        K(1, 2*l+1), sbsize3)
          !
          if (overlap) then
             !call threaded_assign( buffer1,  sp, kdimx, l, col_idx )
             CALL ZGEMM('C','N', l, l, kdim, C_ONE, buffer, kdimx, sp(1,(j-1)*sbsize + 1), kdimx, C_ZERO, &
                                                                                         M(1, 2*l+1), sbsize3)
          else
             !call threaded_assign( buffer1,  p, kdimx, l, col_idx )
             CALL ZGEMM('C','N', l, l, kdim, C_ONE, buffer, kdimx, p(1,(j-1)*sbsize + 1), kdimx, C_ZERO, &
                                                                                         M(1, 2*l+1), sbsize3)
          end if
!civn 
...
!civn 
          ...
          if (overlap) then
             !call threaded_assign( buffer1,  sp, kdimx, l, col_idx )
             CALL ZGEMM('C','N', l, l, kdim, C_ONE, buffer, kdimx, sp(1,(j-1)*sbsize + 1), kdimx, &
                                                                            C_ZERO, M(l+1, 2*l+1), sbsize3)
          else
             !call threaded_assign( buffer1,  p, kdimx, l, col_idx )
             CALL ZGEMM('C','N', l, l, kdim, C_ONE, buffer, kdimx, p(1,(j-1)*sbsize + 1), kdimx, &
                                                                              C_ZERO, M(l+1, 2*l+1), sbsize3)
          end if
!civn
...
!civn 
       ...
       CALL ZGEMM('N','N', kdim, l, l, C_ONE, buffer, kdimx, coord_psi, sbsize, C_ZERO, buffer1, kdimx)
       !psi(:, col_idx(1:l))  = buffer(:,1:l) + p(:,col_idx(1:l))
       psi(:, col_idx(1:l))  = buffer1(:,1:l) + p(:,(j-1)*sbsize + 1: (j-1)*sbsize + l)
       call stop_clock('ppcg:zgemm')
       !
       call start_clock('ppcg:zgemm')
       !call threaded_assign( buffer1,  hpsi, kdimx, l, col_idx )
       ...
       hpsi(:, col_idx(1:l)) = buffer1(:,1:l) + hp(:,(j-1)*sbsize + 1: (j-1)*sbsize + l)
       call stop_clock('ppcg:zgemm')
       !
       if (overlap) then
          call start_clock('ppcg:zgemm')
          ...
          spsi(:, col_idx(1:l)) = buffer1(:,1:l) + sp(:,(j-1)*sbsize + 1: (j-1)*sbsize + l)
          call stop_clock('ppcg:zgemm')
       end if
       !
     ...
     do j=1,nbnd
        if (idx(j)==0) then
           psi (:,j) = C_ZERO 
           hpsi (:,j) = C_ZERO 
           if(overlap) spsi (:,j) = C_ZERO 
           !p(:,j) = C_ZERO 
           !hp(:,j) = C_ZERO
        end if
     end do
!civn (check whether really needed)
     do j = 1, nact 
       if(j .lt.( n_start-1) * sbsize + 1 .or. &
          j .gt. min( n_end * sbsize, nact) ) then
         p(:,j) = C_ZERO 
         hp(:,j) = C_ZERO
         if(overlap) sp(:,j) = C_ZERO
       end if
     end do  
...
!           end if
!        end do
        if(overlap) CALL mp_sum(spsi,inter_bgrp_comm)
        if(overlap) CALL mp_sum(sp,inter_bgrp_comm)
!    end if
    !
    !
