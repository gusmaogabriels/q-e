...
SUBROUTINE lr_orthoUwfc (lflag)
  ...
  !
  IF (Hubbard_manifold=="atomic") THEN
  ...
  ELSEIF (Hubbard_manifold=="ortho-atomic") THEN
  ...
  ELSEIF (Hubbard_manifold=="norm-atomic") THEN
     orthogonalize_wfc = .TRUE.
     normalize_only = .TRUE.
  ...
  ELSE
     ...
     CALL errore ("hp_sphi"," This Hubbard manifold type is not valid",1)
  ENDIF
  !
